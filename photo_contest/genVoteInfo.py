...
def genSemiThread(
    ...
    votes_jury: Dict[int, List[Submission]],
    votes_normal: List[Tuple[int, str]],
    ...
    d.text(
        (375, 50),
        f"submitted by {id2name[photo_author]}",
        anchor="mt",
        font=fnt_italic,
        fill="white",
    )
    ...
    points_per_juror: Dict[str, int] = {
        id2name[i]: 0 for i in votes_jury if int(i) != int(photo_author)
    }
    ...
    for i, (juror, points) in enumerate(
        sorted(points_per_juror.items(), key=lambda x: x[1], reverse=True)
    ):
        fnt = (
            fnt_bold_small
            if points == 12
            else (fnt_regular if points in (8, 10) else fnt_light)
        )
        d.text(
            (50, 395 + i * 25),
    ...
    for voter_id, url in votes_normal:
        if (
            url == photo_url and voter_id != photo_author
        ):  # the 2nd test is for initial votes, because such bad votes are still there yet not counted
    ...
    d.text(
        (400, 100),
        f"{total_points} point{'s' if total_points != 1 else ''} from the global vote",
        anchor="lt",
        font=fnt_bold_small,
        fill="white",
    )
    for i, (voter, points) in enumerate(
        sorted(points_per_voter.items(), key=lambda x: x[1], reverse=True)
    ):
        fnt = (
            fnt_bold_small
            if points == 3
...
def genVoteAnimFinal(
    channel_name: str,
    sub2photoid: Dict[Submission, int],
    id2name: Dict[int, str],
    votes: Dict[int, List[Submission]],
    jurors: Set[int],
    gf1: bool = True,
):
    ...
    def partialPoints(
        i: Optional[int] = None,
    ) -> Tuple[Dict[Submission, int], Dict[Submission, int], Dict[Submission, int]]:
        ...
        for voter, ranking in ordered_votes[
            : (i + 1) if i is not None else len(ordered_votes)
        ]:
    ...
    def drawPoints(
        d: ImageDraw.Draw,
        glob: Dict[Submission, int],
        jury: Dict[Submission, int],
        new_points: Dict[Submission, int],
    ):
        ranked_subs = sorted(
            glob.items(),
            key=lambda x: (x[1], jury.get(x[0], 0), -x[0][2]),
            reverse=True,
        )
        ...
        for i, (sub, nbP) in enumerate(ranked_subs):
            ...
            d.text(
                (x_title + 325, y), str(nbP), anchor="rm", font=fnt_bold, fill="white"
            )

            if i == 0:  # underline the winner
                x1, _, x2, y2 = d.textbbox(
                    (x_title, y), title, anchor="lm", font=fnt_bold
                )
            ...
            if new_points.get(sub):
                ...
                d.text(
                    ...
                    font=fnt_bold
                    if nbP == points[0]
                    else (fnt_bold_small if nbP == points[1] else fnt_regular),
                    fill="white",
                )
    ...
    for i in range(len(points)):
        d.text(
            (50 + 350 * (i >= half_nb_entries), 150 + 125 * (i % half_nb_entries)),
            f"Photo #{i + 1}",
            anchor="lm",
            font=fnt_bold,
            fill="white",
        ...
        d.text(
            (375 + 350 * (i < half_nb_entries), 150 + 125 * (i % half_nb_entries)),
            "0",
            anchor="rm",
            font=fnt_bold,
            fill="white",
