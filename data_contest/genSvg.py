...
def svgCountry(countryName, x, y, currentPointsCountry, newPointsCountry, highlight = False) -> str:
    ...
    <rect x="{293+x*350}" y="{51+y*57}" height="55" width="55" fill="#3F086F" />
    <text x="{77+x*350}" y="{78.5+y*57}" dominant-baseline="middle" fill="white" font-size="25">
    {countryName}
    </text>
    <text x="{322+x*350}" y="{78.5+y*57}" text-anchor="middle" dominant-baseline="middle" fill="white" font-size="23">
    {currentPointsCountry}
    </text>
"""
    if newPointsCountry is not None:
        if highlight: codeCountry += f"""<rect x="{239.5+x*350}" y="{51+y*57}" height="55" width="55" fill="white" />\n"""
        codeCountry += f"""<text x="{268+x*350}" y="{78.5+y*57}" text-anchor="middle" dominant-baseline="middle" fill="{'#3F086F' if highlight else 'white'}" font-size="23">{newPointsCountry}</text>"""
...
def generateSvgs(semi: str):
    ...
    for i, song in enumerate(juryTop):
        ...
        jury_points = currentPoints[song]
    ...
    yield pathPng, "results", None

    standings = sorted(currentPoints.items(), key=lambda x: (x[1], -songs.index(x[0])), reverse=True)
    yield pathPng, "standings", standings

if __name__ == "__main__":
    for _ in generateSvgs("1"):
        input()